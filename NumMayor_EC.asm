...
.data
	num1: 	.asciiz "Introduce el primer número: " 	# Mensaje para pedir el primer número
        ...
        res: 	.asciiz "El número mayor es: " 		# Mensaje para mostrar el resultado
...
main:
	# Se pide el primer número
    	...
    	move $t3, $t0 	# Mueve el valor de $t0 a $t3, asumiendo temporalmente que $t0 es el mayor
    
    	beq $t1, $t2 sonIgual 	# si los dos últimos números son iguales, salta a la etiqueta sonIgual, si no, pasa a la siguiente linea
    
    	bgt $t1, $t3 paso1 	# si no son iguales y si $t1 > $t3, Salta a la etiqueta paso1
    	bgt $t2, $t3 paso2 	# si $t1 no es > $t3 y si $t2 > $t3, Salta a la etiqueta paso2
	...
	sonIgual: 	# Etiqueta para comprobar si los tres números son iguales
		...
		bgt $t1, $t3 paso1 	# Si no son iguales, pregunta si $t1 > $t0(que esta almacenado en $t3), entonces salta a paso1
		j resultado		# Si no se cumple ninguna condición salta a la etiqueta resultado
